...
workload = "mix_app"
out_dir = "/Users/xiyuexiang/GoogleDrive/NOCulator/hring/src/bin/sweep_epoch/"
...
filename = '../bin/sweep_epoch.txt'
filename_out = str(filename)
if os.path.exists(filename_out) == True:
	os.remove(filename_out)
fo_out = open(filename_out, "a")

fo_out.write('\n\n' + 'sweep epoch for calculating slowdown ' + '\n\n')
fo_out.write('Epoch'.ljust(15) + 'w_speedup'.ljust(15) + 'h_speedup'.ljust(15))
fo_out.write('\n')

for sim_index in range(300000,500000, 10000):
	out_file = "sim_" + str(sim_index) + ".out"
	command_line = "mono ../bin/sim.exe -config " + workload_dir + "config_0.txt -output " + out_dir + out_file + " -workload " + workload_dir + workload + ' 3 ' + "-slowdown_epoch " + str(sim_index)
	...
	fo_out.write('\n')
	fo_out.write(str(sim_index).ljust(15) + str("%.2f" % weighted_speedup).ljust(15) + str("%.2f" % harmonic_speedup).ljust(15))
	fo_out.write('\n')
